...
function ItemRow({ item, products, onUpdate, onRemove, canRemove }) {
  ...
  return (
    ...
    >
      ...
      {/* Sasia */}
      <input
        className="form-control text-center text-lg font-semibold px-1 py-2 mt-0.5"
      ...
      {/* Çmimi */}
      <input
        className="form-control text-right text-lg font-semibold px-2 py-2 mt-0.5"
      ...
      {/* Line total */}
      <div className={`text-right text-lg font-bold py-2 px-1 mt-0.5 ${lineTotal > 0 ? 'text-gray-800' : 'text-gray-300'}`}>
        €{fmtN(lineTotal)}
      </div>
      ...
      <div className="bg-gray-50 rounded-xl border border-gray-100 px-4 py-3 mb-4 space-y-2">
        ...
        <div className="flex items-center justify-between text-lg">
          <div className="flex items-center gap-2 text-gray-500 font-medium">
            <span>Zbritja</span>
            <input
              className="w-16 border border-gray-200 rounded-lg px-2 py-1.5 text-lg text-right outline-none focus:border-blue-400 bg-white font-semibold"
            ...
            />
            <select
              className="border border-gray-200 rounded-lg px-2 py-1.5 text-lg outline-none focus:border-blue-400 bg-white text-gray-700 font-semibold"
              value={discount.type}
          ...
          <span className={`font-semibold text-lg ${discAmount > 0 ? 'text-red-500' : 'text-gray-300'}`}>
            -{discAmount > 0 ? `€${fmtN(discAmount)}` : '€0'}
          </span>
        </div>
        <div className="flex items-center justify-between pt-2 border-t border-gray-200">
          <span className="font-bold text-gray-800 text-lg">Total (€)</span>
          <span className="text-3xl font-bold text-blue-700">€{fmtN(total)}</span>
        </div>
      </div>
