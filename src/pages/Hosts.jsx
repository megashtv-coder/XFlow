...
const inputCls = 'w-full px-3 py-2 border border-gray-200 dark:border-gray-700 rounded-lg text-sm bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-red-500'
          ...
          <div className="flex flex-wrap gap-1.5 mb-2">
            {FLAG_SUGGESTIONS.map(f => (
              <button
                ...
                className={`w-9 h-9 rounded-lg text-lg border transition-colors ${flag === f ? 'border-red-400 bg-red-50 dark:bg-red-900/30' : 'border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700'}`}
              >{f}</button>
            ))}
          </div>
    ...
    >
      <div className="space-y-4">
        ...
        <label className="flex items-center gap-2 text-xs font-semibold text-gray-600 dark:text-gray-300 cursor-pointer select-none">
          <input type="checkbox" checked={isPopular} onChange={e => setIsPopular(e.target.checked)} className="w-4 h-4 rounded border-gray-300 dark:border-gray-600 text-red-500 focus:ring-red-500" />
          Shëno si "Popullore"
        </label>
        {err && <p className="text-xs text-red-600 dark:text-red-400 font-semibold">{err}</p>}
      </div>
  ...
  return (
    <div className={`group flex items-center gap-1 rounded-xl border px-3 py-2 transition-colors ${
      copied
        ...
        : 'border-gray-100 dark:border-gray-700 hover:border-red-200 dark:hover:border-red-900/50 hover:bg-gray-50 dark:hover:bg-gray-900/40'
    }`}>
      <button onClick={copyLink} className="min-w-0 flex-1 text-left" title="Kliko për të kopjuar">
        <div className="flex items-center gap-1.5 min-w-0">
          ...
          {host.isPopular && <span className="text-[9px] font-bold px-1.5 py-0.5 rounded-md bg-red-50 dark:bg-red-900/30 text-red-600 dark:text-red-400 flex-shrink-0">Popullore</span>}
        </div>
      ...
      <div className="flex items-center gap-0.5 flex-shrink-0 opacity-0 group-hover:opacity-100 focus-within:opacity-100 transition-opacity">
        ...
        <a href={host.url} target="_blank" rel="noopener noreferrer" onClick={e => e.stopPropagation()}
          className="p-1 rounded-lg text-gray-400 dark:text-gray-500 hover:text-red-600 dark:hover:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/30" title="Hap linkun">
        ...
        <button onClick={() => onDelete(host)} className="p-1 rounded-lg text-gray-400 dark:text-gray-500 hover:text-red-600 dark:hover:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/30" title="Fshi">
          <Trash2 size={13} />
        </button>
      </div>
    </div>
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-center gap-2">
        <div className="relative flex-1 min-w-[220px] max-w-xl">
          ...
          <input
            className="w-full pl-9 pr-9 py-2 rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 text-gray-900 dark:text-white placeholder-gray-400 text-xs font-medium outline-none focus:ring-2 focus:ring-red-50 dark:focus:ring-red-900/20 focus:border-red-400 transition-all"
          ...
          return (
            <button
              ...
              className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-colors ${
                active ? 'bg-red-600 text-white' : 'bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 text-gray-600 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700'
              }`}
            >
              {c.flag && <span className="mr-1">{c.flag}</span>}{c.name} <span className={active ? 'text-red-100' : 'text-gray-400'}>({count})</span>
            </button>
          )
        })}
      </div>
          ...
          <div key={cat.id} className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 overflow-hidden flex flex-col">
              ...
              <div className="flex items-center gap-1">
                ...
                {!DEFAULT_IDS.has(cat.id) && (
                  <button onClick={() => deleteCategory(cat)}
                    className="p-1.5 rounded-lg text-gray-400 dark:text-gray-500 hover:text-red-600 dark:hover:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/30" title="Fshi kategorinë">
                    <Trash2 size={13} />
                  </button>
                )}
              </div>
            ...
            <div className="p-3 flex flex-col gap-2 flex-1">
              ...
              <button
                ...
                className="flex items-center justify-center gap-1 rounded-xl border-2 border-dashed border-gray-200 dark:border-gray-700 py-2 text-xs font-semibold text-gray-400 dark:text-gray-500 hover:border-red-300 hover:text-red-600 dark:hover:text-red-400 transition-colors"
              >
