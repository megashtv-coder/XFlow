...
const keyOf = raw => normalizeReferenceName(raw).toLowerCase()
...
export default function PaymentNames() {
  ...
  const [view,   setView]   = useState(() => { try { return localStorage.getItem('xflow_names_view') || 'grid' } catch { return 'grid' } })
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-2.5">
        {[['all', 'Krejt emrat', ''], ['ok', 'Në rregull', 'text-emerald-600'], ['warn', 'Afër limitit', 'text-amber-600'], ['bad', 'Të bllokuar', 'text-red-600']].map(([k, label, cls]) => (
          <button key={k} onClick={() => setFilter(k)}
            className={`text-left rounded-2xl border bg-white dark:bg-gray-800 px-4 py-3 transition-colors ${filter === k ? 'border-red-400 ring-2 ring-red-100 dark:ring-red-900/30' : 'border-gray-200 dark:border-gray-700'}`}>
      ...
      <div className="flex flex-wrap items-center gap-2">
        <div className="relative flex-1 min-w-[220px] max-w-md">
          ...
          <input
            className="w-full pl-9 pr-9 py-2 rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 text-gray-900 dark:text-white placeholder-gray-400 text-xs font-medium outline-none focus:border-red-400"
        ...
        <div className="inline-flex rounded-xl border border-gray-200 dark:border-gray-700 overflow-hidden bg-white dark:bg-gray-900">
          <button onClick={() => setViewP('grid')} className={`px-2.5 py-2 ${view === 'grid' ? 'bg-red-600 text-white' : 'text-gray-500'}`} title="Kartela"><LayoutGrid size={14} /></button>
          <button onClick={() => setViewP('list')} className={`px-2.5 py-2 ${view === 'list' ? 'bg-red-600 text-white' : 'text-gray-500'}`} title="Tabelë"><ListIcon size={14} /></button>
        </div>
