    ...
    <div>
      <div className="flex items-center gap-1.5">
        <button type="button" onClick={() => scroll(-1)}
          className="flex-shrink-0 w-7 h-7 rounded-full border border-gray-200 flex items-center justify-center text-gray-400 hover:bg-gray-50 transition-colors dark:border-gray-700 dark:text-gray-500 dark:hover:bg-gray-900/50">
          <ChevronLeft size={14} />
        </button>
...
export function ExpenseModal({ expense, onClose, isFormPage }) {
  ...
  const formContent = (
    <>
      ...
      {/* Recurring */}
      <div className="border border-gray-100 rounded-xl p-4 bg-gray-50/50 dark:border-gray-700">
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2">
            <RefreshCw size={15} className={form.recurring ? 'text-red-500' : 'text-gray-300'} />
            <span className="text-sm font-semibold text-gray-700 dark:text-gray-200">Shpenzim i përsëritur</span>
            {form.recurring && (
          ...
          <button type="button" onClick={() => set('recurring', !form.recurring)}
            ...
            <span className={`absolute top-0.5 w-5 h-5 bg-white rounded-full shadow transition-transform dark:bg-gray-800 ${
        ...
        {form.recurring && (
          <div>
            <p className="text-xs text-gray-400 mb-2 dark:text-gray-500">Frekuenca e përsëritjes:</p>
            <div className="flex gap-2">
              {FREQ_OPTIONS.map(f => (
    ...
    <Modal
      ...
      footer={isRecurringParent ? (
        <>
          ...
          <button className="btn bg-red-500 hover:bg-red-600 text-white" onClick={delSeries}>Fshi krejt serinë</button>
        </>
      ) : (
        <>
          <button className="btn btn-outline" onClick={onClose}>Anulo</button>
          <button className="btn bg-red-500 hover:bg-red-600 text-white" onClick={delOne}>Fshi</button>
        </>
      )}
    >
    ...
    <div className="fixed inset-0 bg-black/30 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-md p-6 dark:bg-gray-800">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-xl font-bold text-gray-800 dark:text-gray-100">Eksporto Shpenzimet</h2>
      ...
      {paged.length === 0 ? (
        ...
        <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200/90 dark:border-gray-700 shadow-sm overflow-hidden hidden sm:block">
          ...
          <table className="w-full text-sm min-w-[480px]">
            <thead className="sticky top-0 z-10">
              <tr className="border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900/60">
                {[
                  ...
                  <th key={col.key} className={`table-th cursor-pointer select-none hover:text-gray-700 dark:hover:text-gray-200 ${col.cls}`}
                ...
                <th className="table-th hidden md:table-cell cursor-pointer select-none hover:text-gray-700 dark:hover:text-gray-200"
                ...
                <th className="table-th text-right cursor-pointer select-none hover:text-gray-700 dark:hover:text-gray-200"
            ...
            <tbody>
              {paged.map(e => (
                <tr key={e.id} className="hover:bg-gray-50 dark:hover:bg-gray-900/40 transition-colors group">
                  ...
                  <td className="table-td text-right font-mono font-bold text-red-600 dark:text-red-400">
                    - {fmt(e.amount)}
                  </td>
                  <td className="table-td">
