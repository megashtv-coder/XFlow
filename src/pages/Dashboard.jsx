...
const PALETTE = [
  '#dc2626', '#2563eb', '#7c3aed', '#059669', '#d97706', '#0891b2',
...
/* Ngjyrë e qëndrueshme për çdo emër, e dallueshme nga të tjerat në ekran.
   Nis nga hash-i i emrit dhe kalon te slot-i i parë i lirë nëse është i zënë. */
...
/* ── Kartelë KPI me krahasim ndaj periudhës së kaluar ── */
const DELTA_TONE = {
...
function KpiCard({ icon: Icon, iconBg, iconColor, label, value, delta, deltaUp, deltaTone = 'neutral', ctx, onClick, hidden }) {
  ...
  const interactive = onClick
    ...
    : 'hover:-translate-y-0.5 hover:shadow-md hover:shadow-gray-200/60 dark:hover:shadow-black/20'
  // Shigjeta ndjek DREJTIMIN real të numrit (rritje/rënie), ngjyra e pilulës
  // (deltaTone) jep GJYKIMIN nëse ai drejtim është i mirë a i keq për këtë
  // metrikë — për shpenzimet rritja është e keqe (kuq) por shigjeta prapë
...
function YoYChart({ title, sub, data, curKey, prevKey, color, softColor, gradId, curLabel, prevLabel, fmt, hidden }) {
  ...
  const Tip = ({ active, payload, label }) => {
    ...
    return (
      <div className="bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 rounded-xl shadow-lg px-4 py-3 text-xs min-w-[175px]">
        ...
        <div className="flex items-center justify-between gap-4">
          ...
          <span className="font-bold text-gray-800 dark:text-gray-100">{c === null || c === undefined ? '—' : fmt(c)}</span>
        </div>
        <div className="flex items-center justify-between gap-4 mt-1">
          <span className="flex items-center gap-1.5 text-gray-400 dark:text-gray-500">
            <span className="w-2 h-2 rounded-full" style={{ background: softColor }}/>{prevLabel}
          </span>
          <span className="font-semibold text-gray-500 dark:text-gray-400">{fmt(p)}</span>
        </div>
        {d !== null && (
          <p className={`mt-2 pt-2 border-t border-gray-50 dark:border-gray-700 font-bold ${d >= 0 ? 'text-emerald-600 dark:text-emerald-400' : 'text-red-500 dark:text-red-400'}`}>
  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 flex flex-col h-full">
      <div className="flex items-start justify-between gap-3 px-5 py-4 border-b border-gray-50 dark:border-gray-700 flex-wrap">
        <div>
          <p className="text-sm font-bold text-gray-800 dark:text-gray-100">{title}</p>
          <p className="text-xs text-gray-400 dark:text-gray-500 mt-0.5">{sub}</p>
        </div>
        <div className="flex gap-3 text-[11px] text-gray-500 dark:text-gray-400 font-medium">
      ...
      {/* Shirit përmbledhës — krahasimi lexohet pa lëvizur mausin */}
...
const TASK_STATUS_STYLE = {
  overdue: { label: 'Vonuar', cls: 'bg-red-50 dark:bg-red-900/30 text-red-600 dark:text-red-400', rail: '#dc2626' },
  today:   { label: 'Sot',    cls: 'bg-amber-50 dark:bg-amber-900/30 text-amber-600 dark:text-amber-400', rail: '#d97706' },
  soon:    { label: 'Afër',   cls: 'bg-sky-50 dark:bg-sky-900/30 text-sky-600 dark:text-sky-400', rail: '#0284c7' },
}
...
function TasksUrgentPanel({ tasks, loading, hidden, onToggle, onDelete, onSeeAll }) {
  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 flex flex-col h-full">
      <div className="flex items-center justify-between gap-3 px-5 py-4 border-b border-gray-50 dark:border-gray-700">
        <div>
          <p className="text-sm font-bold text-gray-800 dark:text-gray-100">Detyra afër afatit</p>
          <p className="text-xs text-gray-400 dark:text-gray-500 mt-0.5">Brenda 3 ditësh ose vonuar</p>
        </div>
        <span className="font-mono text-sm font-bold px-2.5 py-1 rounded-full bg-amber-50 dark:bg-amber-900/30 text-amber-600 dark:text-amber-400">
      ...
      <div className="flex-1 px-2.5 py-1.5 flex flex-col">
        {loading ? (
          <p className="text-xs text-gray-400 dark:text-gray-500 text-center py-8 italic">Duke ngarkuar...</p>
        ) : preview.length === 0 ? (
          <p className="text-xs text-gray-400 dark:text-gray-500 text-center py-8">S'ka detyra urgjente ✓</p>
        ) : (
          preview.map(t => {
            const st = TASK_STATUS_STYLE[statusOf(t)]
            return (
              <div key={t.id} className="flex gap-2.5 px-1.5 py-2 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-900/40 transition-colors">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-1.5">
                    ...
                    <button
                      ...
                      className="w-[21px] h-[21px] flex items-center justify-center rounded-md bg-emerald-50 dark:bg-emerald-900/30 text-emerald-600 dark:text-emerald-400 hover:bg-emerald-500 hover:text-white transition-colors flex-shrink-0"
                    >
                    ...
                    <button
                      ...
                      title="Fshi detyrën"
                      className="w-[21px] h-[21px] flex items-center justify-center rounded-md bg-gray-100 dark:bg-gray-700 text-gray-400 dark:text-gray-500 hover:bg-red-50 dark:hover:bg-red-900/30 hover:text-red-500 dark:hover:text-red-400 transition-colors flex-shrink-0"
                    >
                      <Trash2 size={12} />
                    </button>
                  </div>
      ...
      <div className="px-5 py-3 border-t border-gray-50 dark:border-gray-700">
        <button
          onClick={onSeeAll}
          className="w-full flex items-center justify-center gap-1.5 text-xs font-bold text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg py-1.5 transition-colors"
        >
...
export default function Dashboard() {
  const { invoices, customers, expenses, payments, navigate, fmt, currentUser, logActivity, showToast } = useApp()
  ...
  const revBg   = revDelta === null ? '#fef2f2' : revUp ? '#ecfdf5' : '#fef2f2'
  const revFg   = revDelta === null ? '#dc2626' : revUp ? '#059669' : '#dc2626'
  ...
  const { catData, catTotal, top5Types, groupByType } = useMemo(() => {
    ...
    const typeGroups = {}
    filtered.forEach(e => {
      const cat  = normalizeExpenseType(e.category || e.type || 'Tjera')
      const type = normalizeExpenseType(e.type || e.category || 'Tjera')
    ...
    const typeList = Object.entries(typeGroups)
      .map(([name, g]) => ({ name, value: g.value, cats: g.cats })).sort((a, b) => b.value - a.value)

    /* Kategoria shpesh nuk plotësohet dhe çdo shpenzim bie te "Tjera" —
       atëherë grupojmë sipas llojit që grafiku të ketë kuptim. */
  ...
  return (
    <div className="space-y-6 max-w-6xl">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        {[
          { icon: FilePlus, title: 'Krijo Faturë', sub: 'Faturë e re shpejt', action: openInvoiceModal,
            border: 'border-red-400 dark:border-red-800', hover: 'hover:bg-red-50 dark:hover:bg-red-900/10', icoBg: 'bg-red-50 dark:bg-red-900/20', icoFg: 'text-red-500 dark:text-red-400' },
          { icon: UserPlus, title: 'Shto Klient', sub: 'Regjistro klient të ri', action: openCustomerModal,
            border: 'border-emerald-400 dark:border-emerald-800', hover: 'hover:bg-emerald-50 dark:hover:bg-emerald-900/10', icoBg: 'bg-emerald-50 dark:bg-emerald-900/20', icoFg: 'text-emerald-500 dark:text-emerald-400' },
          { icon: ReceiptText, title: 'Shpenzim i ri', sub: 'Regjistro shpenzim', action: openExpenseModal,
            border: 'border-orange-400 dark:border-orange-800', hover: 'hover:bg-orange-50 dark:hover:bg-orange-900/10', icoBg: 'bg-orange-50 dark:bg-orange-900/20', icoFg: 'text-orange-500 dark:text-orange-400' },
        ].map(({ icon: Icon, title, sub, action, border, hover, icoBg, icoFg }) => (
          <button key={title} onClick={action}
            className={`group flex items-center gap-3 bg-white dark:bg-gray-800 border-[1.5px] ${border} ${hover} rounded-2xl p-4 text-left transition-all duration-150 hover:-translate-y-0.5 hover:shadow-md hover:shadow-gray-200/60 dark:hover:shadow-black/20 active:scale-95`}>
            ...
            <span className="min-w-0">
              <span className="block text-sm font-bold text-gray-800 dark:text-gray-100 truncate">{title}</span>
              <span className="block text-xs text-gray-400 dark:text-gray-500 truncate">{sub}</span>
            </span>
          </button>
        ))}
      </div>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 p-6">
        ...
        <div className="flex flex-wrap items-center justify-between gap-4 pb-4 mb-6 border-b border-gray-100 dark:border-gray-700">
          <div className="flex items-center gap-2.5">
            <span className="w-9 h-9 rounded-[9px] bg-red-50 dark:bg-red-900/20 text-red-600 dark:text-red-400 flex items-center justify-center flex-shrink-0">
              <PieIcon size={17} />
            </span>
            <div>
        ...
        {catData.length === 0 ? (
          <p className="text-sm text-gray-400 dark:text-gray-500 text-center py-10 italic">Nuk ka shpenzime për këtë periudhë</p>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-[220px_1fr] gap-8 items-center">
            ...
            <div className={`relative h-[200px] flex items-center justify-center transition-all duration-200 ${hideData ? 'blur-md select-none pointer-events-none' : ''}`}>
              ...
              <div className="absolute text-center pointer-events-none">
                {activeCat === null ? (
                  <>
                    <p className="text-[11px] text-gray-500 dark:text-gray-400 uppercase tracking-wider font-medium">Total</p>
                    <p className="text-xl font-bold text-gray-900 dark:text-gray-100 mt-0.5">{fmt(catTotal)}</p>
                  </>
                ) : (
                  <>
                    <p className="text-[11px] text-gray-500 dark:text-gray-400 uppercase tracking-wider font-medium">{catData[activeCat].name}</p>
                ...
                return (
                  <div key={i} className="flex flex-col gap-1.5">
                    <div className="flex items-center justify-between gap-3 text-[13px]">
                      <span className="flex items-center gap-2 min-w-0 font-medium text-gray-700 dark:text-gray-200">
                        <span className="w-2 h-2 rounded-full flex-shrink-0"
                              style={{ background: t.color }} title={t.category} />
