...
export default function Settings() {
  ...
  // Backup i menjëhershëm — krijon një auto-backup tani (shkarkim + listë) pa pritur ciklin 48-orësh
  const handleCreateBackupNow = async () => {
    setIsBackingUp(true)
    const result = await BackupService.createAutoBackup(buildFullAppState())
    if (result.success) {
      localStorage.setItem('xflow_last_backup_time', Date.now().toString())
  ...
  const applyRestoreData = (data) => {
    ...
    if (data.companyData) {
      localStorage.setItem('xflow_company_data', JSON.stringify(data.companyData))
      setCompanyData(data.companyData)
    }
    if (data.notifAdvanceDays != null) {
      localStorage.setItem('xflow_notif_advance_days', data.notifAdvanceDays.toString())
      setAdvanceDays(data.notifAdvanceDays)
    }
    if (data.messageLogs) {
      localStorage.setItem('xflow_message_logs', JSON.stringify(data.messageLogs))
    }
  }
      ...
      <div className="max-w-2xl space-y-5">
          ...
          <div className="bg-white rounded-xl border border-gray-100 overflow-hidden dark:bg-gray-800 dark:border-gray-700">
            <div className="flex items-center justify-between px-5 py-3.5 border-b border-gray-50 dark:border-gray-700">
              <div>
                ...
                <p className="text-xs text-gray-400 mt-0.5 dark:text-gray-500">Çdo të dhënë (fatura, klientë, furnitorë, detyra, cilësime...) si file i kompresuar</p>
              </div>
              <button
            ...
            <div className="flex items-center justify-between px-5 py-3.5 border-b border-gray-50 dark:border-gray-700">
              <div>
                <p className="text-sm font-semibold text-gray-800 dark:text-gray-100">Krijo Backup Tani</p>
                <p className="text-xs text-gray-400 mt-0.5 dark:text-gray-500">Shkarko + shto në listën e auto-backup-eve, pa pritur ciklin 48-orësh</p>
              </div>
              <button
            ...
            <div className="flex items-center justify-between px-5 py-3.5">
              <div>
                ...
                <p className="text-xs text-gray-400 mt-0.5 dark:text-gray-500">Importo nga file .json ose .json.gz</p>
              </div>
              <button
        ...
        <div>
          <div className="flex items-center gap-2 mb-2 px-1">
            ...
            <p className="text-xs font-bold text-gray-400 uppercase tracking-widest dark:text-gray-500">Auto-backup-et (çdo 48 orë, {autoBackups.length}/3 të fundit)</p>
          </div>
          <div className="bg-white rounded-xl border border-gray-100 overflow-hidden dark:bg-gray-800 dark:border-gray-700">
            {autoBackups.length === 0 ? (
              <div className="px-5 py-8 text-center">
                <p className="text-sm text-gray-500 dark:text-gray-400">Nuk ka auto-backup-e ende</p>
                <p className="text-xs text-gray-400 mt-1 dark:text-gray-500">Auto-backup-et do të krijohen automatikisht çdo 48 orë (edhe u shkarkojnë vetë si file .gz)</p>
              </div>
            ) : (
