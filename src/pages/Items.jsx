        ...
        <div className="sm:hidden space-y-2 mb-4">
          {paged.map(item => {
            const margin = item.purchasePrice > 0
              ...
              : null
            return (
              <div key={item.id} className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 shadow-sm p-3">
                <div className="flex justify-between items-start gap-2">
                  {/* Col 1: Emri + llogaria + furnitori */}
                  <div className="flex-1 min-w-0 cursor-pointer" onClick={() => openEdit(item)}>
                    <p className="font-bold text-gray-900 dark:text-white text-sm truncate hover:text-red-500 dark:hover:text-red-400 transition-colors">{item.name}</p>
                    <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">{accountLabel(item.account)}</p>
                    {item.vendor && <p className="text-xs text-gray-400 dark:text-gray-500 font-semibold">{item.vendor}</p>}
                  </div>

                  {/* Col 2: Çmimi + marzha */}
                  <div className="text-right">
                    <p className="font-mono font-bold text-gray-900 dark:text-white text-sm">{fmt(item.salePrice)}</p>
                    {pinUnlocked ? (
                      margin !== null && (
                      ...
                      <span className="text-xs text-gray-300 dark:text-gray-600 cursor-pointer" onClick={() => setShowPin(true)}>{masked}</span>
                    )}
                  </div>
