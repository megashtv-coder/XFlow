...
export async function buildBackupPayload(appState) {
  ...
  return {
    ...
    data: {
      ...
      tasks,
      companyData:      readLocalJSON('xflow_company_data', null),
      notifAdvanceDays: readLocalJSON('xflow_notif_advance_days', null),
      messageLogs:      readLocalJSON('xflow_message_logs', []),
    },
  }
}
...
export async function downloadBackup(appState) {
  try {
    ...
    link.href = url
    link.download = `xflow-backup-${timestamp}.json${compressed ? '.gz' : ''}`
...
export async function downloadAutoBackupToDisk(backup) {
  try {
    ...
    const filename = `xflow-autobackup-${timestamp}.json${compressed ? '.gz' : ''}`
