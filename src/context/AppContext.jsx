...
export function AppProvider({ children }) {
  ...
  const [invoicesHidden,     setInvoicesHidden]     = useState(() => readHidden('xflow_invoices_hidden'))
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      // gjendja E ZGJERUAR (siç ndodhte më parë), do të ishte objekti I NJËJTË
      // me `expenses` — diffSync do të shihte "asnjë ndryshim" dhe s'do të
      // upsertonte KURRË instancat e reja të shpenzimeve të rregullta.
      const rawLoadedExpenses = loadedExpenses

      // new Date('YYYY-MM-DD') e interpreton si mesnatë UTC — në zona orare
      // me offset negativ (jo Shqipëria/Kosova, por dev/CI mund të jetë),
      // .getDate()/.getMonth() lokal e kthen një ditë mbrapa. Parsojmë/formatojmë
      // gjithmonë në kohë lokale, jo përmes ISO/UTC, që data të mos zhvendoset.
      ...
      const getNextOccurrence = (anchorDate, freq, fromDate) => {
        ...
        if (freq === 'Vjetore') {
          ...
          if (d.getMonth() !== anchorMonth) d = new Date(y, anchorMonth + 1, 0) // p.sh. 29 shkurt në vit jo-tranzicion
          return d
        }
        // Mujore (default)
        ...
        if (d.getMonth() !== m) d = new Date(y, m + 1, 0) // p.sh. 31 në muaj me 30 ditë
        return d
      }
      ...
      // nga data origjinale e krijimit. Kur një prind s'ka ende
      // lastGeneratedDate (i porsakrijuar, ose ekzistues nga para këtij
      // rregullimi), thjesht caktohet lastGeneratedDate = sot pa gjeneruar
      // asgjë këtë herë — kështu asnjë muaj/vit i kaluar s'rikrijohet
      // retroaktivisht ("pa backfill historik"); gjenerimi fillon vetëm nga
      // occurrence-i i ardhshëm pas kësaj.
      const expandRecurringExpenses = (expenses) => {
        ...
        expenses.filter(e => e.recurring && !e.parentId).forEach(parent => {
          ...
          let guard = 0 // siguri kundër loop-ave të pafundme
          while (guard++ < 1000) {
      ...
      if (sett.data?.length) {
        ...
        const etRow = sett.data.find(r => r.key === 'expenseTypes')
  ...
  useEffect(() => {
    if (!canSync || !supabase) return
    if (JSON.stringify(prevSL.current) === JSON.stringify(stripeLinks)) return
    prevSL.current = stripeLinks
    supabase.from('settings').upsert({ key: 'stripeLinks', value: stripeLinks }).then()
  }, [stripeLinks, canSync])

  useEffect(() => {
    if (!canSync || !supabase) return
    if (JSON.stringify(prevHosts.current) === JSON.stringify(hosts)) return
  ...
  useEffect(() => {
    if (!canSync || !supabase) return
    if (JSON.stringify(prevET.current) === JSON.stringify(expenseTypes)) return
